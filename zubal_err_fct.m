function [mean_abs_error_method] = zubal_err_fct(mask_file_name, method, mag_field_sim, plane, cross_sect)
%
% zubal_err_fct computes the mean absolute error for a certain mask in the zubal
...
% and the true magnetic field and finally we divide tis substraction with 
...
% have a 2D data set containing the absolute error for each element of your
% matrix. We calculate the mean value of the 2D data set and the output 
% is the mean absolute error for a specify plane and cross section.
%
% _SYNTAX_
% 
% [mean_abs_error_method] = err_fct(mask_file_name, method, mag_field_sim, plane, cross_sect)
%
% _DESCRIPTION_
%
% _INPUT ARGUMENTS_
...
% _OUTPUTS_
%
%    mean_abs_error_method 
%      mean absolute error for a specify plane and cross section
% 
%
% _EXAMPLE_
% 
...
% calculating the efficacity of both methods using the absolute error
switch plane
        ...
        for i = 1 : dim2
            for j = 1 : dim3
                if mask_dBz(cross_sect,i,j)==0
                    abs_error_method(i,j) = 0;
                else
                    abs_error_method(i,j) = abs((mask_dBz(cross_sect,i,j) - mask_method(cross_sect,i,j))/...
                                            mask_dBz(cross_sect,i,j)); % absolute error 
                end
            end
        end
        mean_abs_error_method = sum(abs_error_method(:,:), 'all')/nnz(mask(cross_sect,:,:));
        ...
        for i = 1 : dim1
            for j = 1 : dim3
                if mask_dBz(i,cross_sect,j)==0
                    ...
                    abs_error_method(i,j) = abs((mask_dBz(i,cross_sect,j) - mask_method(i,cross_sect,j))/...
                                            mask_dBz(i,cross_sect,j)); % absolute error
                end
            end
        end
        mean_abs_error_method = sum(abs_error_method(:,:), 'all')/nnz(mask(:,cross_sect,:));
    ...
    case 'axial'
        ...
        for i = 1 : dim1
            for j = 1 : dim2
                ...
                abs_error_method(i,j) = abs((mask_dBz(i,j,cross_sect) - mask_method(i,j,cross_sect))/...
                                        mask_dBz(i,j,cross_sect)); % absolute error 
                end
            end
        end
        mean_abs_error_method = sum(abs_error_method(:,:), 'all')/nnz(mask(:,:,cross_sect));
