...
list_SNR = [50 200 250 300 400]; % linspace(100,500,17); % linspace(0,300,21); % get different SNR
...
zubal_vol.simulate_measurement(4, [0.00105 0.00246 0.00369 0.00492 0.00615 0.00738], list_SNR(k));
...
[dual_echo_delf] = +imutils.b0.dual_echo(compl_vol(:,:,:,1:2), [0.00105 0.00246]);
[multi_echo_delf] = +imutils.b0.multiecho_linfit(compl_vol, [0.00105 0.00246 0.00369 0.00492 0.00615 0.00738]); 
...
% 'meanvalue_and_niftifile' or 'meanvalue' or 'niftifile'
[percent_diff_dual] = zubal_err_fct('zubal_pre_lobes_mask.nii.gz', dual_echo_b0_ppm, zubal_dBz.volume, 'meanvalue_and_niftifile', 'dual_echo_percent_diff');
mean_rel_error_dual = [mean_rel_error_dual, percent_diff_dual];
 
[percent_diff_multi] = zubal_err_fct('zubal_pre_lobes_mask.nii.gz', multi_echo_b0_ppm, zubal_dBz.volume, 'meanvalue_and_niftifile', 'multi_echo_percent_diff');
...
title({'SNR variation (test2)'},{'Mean relative error from 100 to 500 with increment of 25'})
xlabel('SNR')
ylabel('error')
grid on
hold off
